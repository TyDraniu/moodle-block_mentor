...
require_once($CFG->dirroot.'/blocks/fn_mentor/filters/user_filter_forms.php');
...
class fn_user_filtering {
    ...
    public function __construct($fieldnames = null, $baseurl = null, $extraparams = null, $filtertype = '', $returnurl = '') {
        ...
        $filtersession = $this->_filtertype.'_user_filtering';

        if (!isset($SESSION->$filtersession)) {
        ...
        $extraparams['filtertype'] = $this->_filtertype;
        ...
        if ($adddata = $this->_addform->get_data()) {
            foreach ($this->_fields as $fname => $field) {
                ...
                if (!array_key_exists($fname, $SESSION->$filtersession)) {
                    $SESSION->{$filtersession}[$fname] = array();
                }
                $SESSION->{$filtersession}[$fname][] = $data;
            }
            // Clear the form.
            $_POST = array();
            $this->_addform = new fn_user_add_filter_form($baseurl, ['fields' => $this->_fields, 'extraparams' => $extraparams]);
            ...
            if (!empty($adddata->removeall)) {
                ...
                foreach ($adddata->filter as $fname => $instances) {
                    foreach ($instances as $i => $val) {
                        ...
                        unset($SESSION->{$filtersession}[$fname][$i]);
                    }
                    if (empty($SESSION->{$filtersession}[$fname])) {
                        unset($SESSION->{$filtersession}[$fname]);
                    }
                }
            }
    ...
    public function get_sql_filter($extra='', array $params=null) {
        ...
        if (!empty($SESSION->$filtersession)) {
            foreach ($SESSION->$filtersession as $fname => $datas) {
    ...
    public function display_active_filters() {
        ...
        if (($this->_filtertype != 'mentor') && ($this->_filtertype != 'selectedmentor') && ($this->_filtertype != 'mentee')) {
            return $output;
        }
        ...
        if ($output) {
            $output = html_writer::div(
                html_writer::div(get_string('actfilterhdr', 'filters')).
