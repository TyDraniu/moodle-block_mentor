...
$PAGE->navbar->add(get_string('assign', 'block_fn_mentor'));

$PAGE->requires->string_for_js('pleaseselectamentor', 'block_fn_mentor');
$PAGE->requires->jquery();
$PAGE->requires->js('/blocks/fn_mentor/js/assign.js?v='.time());
...
switch ($potentialmenteefilter) {
    case 'all_mentees':
        $students = block_fn_mentor_get_all_students('', false, true);
        break;
    case 'mentees_without_mentor':
        $students = block_fn_mentor_get_students_without_mentor('', true);
        break;
}
...
if (!empty($students)) {
    foreach ($students as $user) {
        $potentialmenteeoptions[$user->id] = $user->firstname . " " . $user->lastname;
    }
}

$selectedmentoroptions = block_fn_mentor_get_mentors_with_mentee();
...
$selectedmentorlabel->text = html_writer::tag('strong', get_string('selectedmentors', 'block_fn_mentor'));

$selectedmenteelabel = new html_table_cell();
$selectedmenteelabel->text = html_writer::tag('strong', get_string('selectedmentees', 'block_fn_mentor'));
...
$potentialmenteeselect->text = html_writer::tag('form',
    ...
    html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'searchmentee', 'value' => $searchmentee)).
    html_writer::select(
        $potentialmenteefilteroptions, 'pmentee', $potentialmenteefilter,
        null,
        array(
